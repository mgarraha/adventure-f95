...
9     A(I)=IAND(A(I),IEOR(SHIFT(IAND(A(I),IA5('@@@@@')),-1),-1))
      ...
      IF(IAND(IEOR(A(J),BLANKS),MSK).EQ.0)GOTO 15
      IF(SECOND.EQ.3)GOTO 20
      IF(SECOND.NE.1)GOTO 10
      MSK=-MASKS(6-K)
      WORD2=IAND(SHIFT(A(J),7*(K-1)),MSK)
     1        +IAND(SHIFT(A(J+1),7*(K-6)),(-2-MSK))
      WORD2X=IAND(SHIFT(A(J+1),7*(K-1)),MSK)
     1        +IAND(SHIFT(A(J+2),7*(K-6)),(-2-MSK))
      SECOND=2
      GOTO 10

15    IF(SECOND.EQ.2)SECOND=3
      IF(SECOND.NE.0)GOTO 10
      SECOND=1
...
C  THE INDEX OF THE LAST NON-BLANK CHAR IN CHARS IS RETURNED IN LENG.
      ...
      DO 2 CH=1,5
      CHARS(POSN)=IAND(WORDS(WORD),MASK)+(BLANK-IAND(BLANK,MASK))
...
C  RESTARTING.  WIZARDS MAY CUT THIS TO A THIRD.
      ...
      PTIME=IAND(PRIMTM,SHIFT(1,T/60)).NE.0
      SOON=.FALSE.
...
10    FROM=FROM+1
      IF(IAND(H,SHIFT(1,FROM)).NE.0)GOTO 10
      IF(FROM.GE.24)GOTO 20
      TILL=FROM
14    TILL=TILL+1
      IF(IAND(H,SHIFT(1,TILL)).EQ.0.AND.TILL.NE.24)GOTO 14
...
11    SHIFT=(IAND(SHIFT,O"377777777777")/2)+J
20    RETURN
30    DO 31 I=1,DIST
      J=0
      IF(IAND(SHIFT,O"200000000000").NE.0)J=O"400000000000"
31    SHIFT=IAND(SHIFT,O"177777777777")*2+J
      RETURN
      END
...
C  FUNCTION I2 TAKES 2-DIGIT ASCII AND YIELDS DECIMAL VALUE.
      ...
      I2(X)=IAND(SHIFT(X,-29),15)*10+IAND(SHIFT(X,-22),15)
      ...
      D=I2(DAT(1))-1
      X=IOR(IAND(IOR(SHIFT(DAT(1),14),SHIFT(DAT(2),-21)),
     1        NOT(O"1004020001")),IA5('-@@@-'))
C  ABOVE FUNNY EXPRESSION GUARANTEES (A) UPPER-CASE, AND (B) BOTTOM BIT OKAY.
      DO 1 MON=1,12
      IF(X.EQ.IA5(MONTHS(MON)))GOTO 2
1     D=D+HATH(MON)
      CALL BUG(28)
